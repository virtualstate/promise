...
export const PromiseAllRejectEarly = Symbol.for(
  "@virtualstate/promise/all/rejectEarly"
);
...
export function all<T, TArgs extends PromiseArgTuple<T>>(
  this: unknown,
  ...promises: TArgs
): TheAsyncThing<PromiseTupleIntermediate<TArgs>, PromiseTuple<TArgs>>;
export function all<T>(
  this: unknown,
  ...promises: PromiseArgs<T>
): TheAsyncThing<T[]>;
export function all<T>(
  this: unknown,
  ...promises: PromiseArgs<T>
...
export function allGenerator<T, TArgs extends PromiseArgTuple<T>>(
  this: unknown,
  ...promises: TArgs
): AsyncIterable<PromiseTupleIntermediate<TArgs>>;
export function allGenerator<T>(
  this: unknown,
  ...promises: PromiseArgs<T>
): AsyncIterable<T[]>;
export async function* allGenerator<T>(
  this: unknown,
  ...promises: PromiseArgs<T>
  ...
  for await (const status of allSettledGenerator(...promises)) {
    ...
    if (
      isLike<PromiseContext>(this) &&
      this?.[PromiseAllRejectEarly] &&
      rejected.length
    ) {
    ...
    yield status.map(
      (status): T => (status?.status === "fulfilled" ? status.value : undefined)
    );
  }
  if (rejected?.length) {
    throw aggregateError(rejected);
  ...
  ): status is PromiseRejectedResult {
