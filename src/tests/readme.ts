...

{
  ...
  // logs rejected
  console.log(
    await all(Promise.resolve(1), Promise.reject(2)).catch(() => "rejected")
  );
}

{
  const wait = (timeout = 1, arg: unknown = undefined) =>
    new Promise((resolve) => setTimeout(resolve, timeout, arg));
