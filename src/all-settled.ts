...
import {
  ...
  PromiseOrAsync,
  PromiseSettledTuple,
  PromiseSettledTupleIntermediate,
} from "./args";

export function allSettled<T, TArgs extends PromiseArgTuple<T>>(
  this: unknown,
  ...promises: TArgs
): TheAsyncThing<
  PromiseSettledTupleIntermediate<TArgs>,
  PromiseSettledTuple<TArgs>
>;
export function allSettled<T>(
  ...promises: PromiseArgs<T>
): TheAsyncThing<PromiseSettledResult<T>[]>;
export function allSettled<T>(...promises: PromiseArgs<T>): TheAsyncThing {
  return anAsyncThing(allSettledGenerator(...promises));
}

export function allSettledGenerator<T, TArgs extends PromiseArgTuple<T>>(
  this: unknown,
  ...promises: TArgs
): AsyncIterable<PromiseSettledTupleIntermediate<TArgs>>;
export function allSettledGenerator<T>(
  ...promises: PromiseArgs<T>
): AsyncIterable<PromiseSettledResult<T>[]>;
export async function* allSettledGenerator<T>(
  ...promises: PromiseArgs<T>
  ...
  async function* map(
    promise: PromiseOrAsync<T>,
    index: number
  ): AsyncIterable<KnownPromiseResult> {
    try {
      ...
      if (!isAsyncIterable(promise)) {
        ...
        return yield [
          index,
          { reason: new Error("Unknown input"), status: "rejected" },
        ];
      }
